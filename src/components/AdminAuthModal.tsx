...
export function AdminAuthModal({ isOpen, onClose }: AdminAuthModalProps) {
  ...
  useEffect(() => {
    ...
    socket.on('otp_sent', (data) => {
      if (data.isSandboxFallback && data.contactValue.toLowerCase() === email.toLowerCase()) {
            ...
            <AnimatePresence>
              {sandboxCode && step === 'otp' && (
                <motion.div 
                  ...
                  className="p-4 bg-amber-50 border border-amber-200 rounded-2xl shadow-sm"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-sm font-bold text-amber-900">Admin Code: <span className="text-xl tracking-[0.2em] ml-2">{sandboxCode}</span></div>
                      <div className="text-[10px] text-amber-500 mt-1">Resend domain unverified. Code shown for testing.</div>
                    </div>
                  </div>
                </motion.div>
              )}
